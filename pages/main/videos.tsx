  ...
  useEffect(() => {
    const fetchYearRoutes = () => {
      ...
      formattedRoutes = formattedRoutes.map(route => {
        switch (route.title) {
          case "First Year":
            return { ...route, title: "1st Year" };
          case "Second Year":
            return { ...route, title: "2nd Year" };
          case "Third Year":
            return { ...route, title: "3rd Year" };
          case "Forth Year":
            return { ...route, title: "4th Year" };
          default:
            return route;
        }
      });
    ...
    <TabsProvider defaultIndex={index}>
      <Tabs>
        {routes.map((route: Route) => (
          <TabScreen key={route.key} label={shortenLabel(route.title)}>
