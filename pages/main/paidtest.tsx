...
const DynamicContent = ({ headerOne, headerTwo, headerThree }: { headerOne: string; headerTwo: string; headerThree: string }) => {
  ...
  const renderItem = ({ item, index }: { item: any; index: number }) => (
    <TouchableOpacity style={styles.card}
        onPress={() => {
            navigation.navigate('paidTestList', { name:item.name,id:item.value } );
       }}>
...
const PaidTestScreen = () => {
  const [index, setIndex] = useState(0);
  const [routes, setRoutes] = useState<Route[]>([]);
  const { headerOneData, loading } = useData();

  useEffect(() => {
    const fetchRoutes = () => {
      const formattedRoutes = headerOneData.map((item: any) => ({ key: item.value.toString(), title: item.name }));
